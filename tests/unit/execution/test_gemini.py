"""Unit tests for the Gemini CLI Runtime Adapter (GeminiRuntimeAdapter)."""
...
@pytest.mark.asyncio
async def test_gemini_initialize(db_session: AsyncSession) -> None:
    """Verify initialization checks for GEMINI_API_KEY settings."""
    adapter = GeminiRuntimeAdapter(db_session, uuid.uuid4())
    await adapter.initialize()
    # Does not crash or raise exception even without keys


@pytest.mark.asyncio
async def test_gemini_validate_fails_no_execution_record(db_session: AsyncSession) -> None:
    """Ensure validate raises error if parent ExecutionRecord does not exist."""
    exec_id = uuid.uuid4()
    adapter = GeminiRuntimeAdapter(db_session, exec_id)
...
@pytest.mark.asyncio
async def test_gemini_validate_success(db_session: AsyncSession) -> None:
    ...
    adapter = GeminiRuntimeAdapter(db_session, exec_record.id)
    await adapter.validate(repository_path=".", command="echo 'hi'")


@pytest.mark.asyncio
async def test_gemini_execute_and_checkpoint(db_session: AsyncSession) -> None:
...
@pytest.mark.asyncio
async def test_gemini_heartbeat(db_session: AsyncSession) -> None:
...
@pytest.mark.asyncio
async def test_gemini_summarize_and_persist(db_session: AsyncSession) -> None:
