...
from nexus.database import Base, get_session
...
from nexus.core.types import TaskStatus, ApprovalStatus, ExitStatus, EventType
...
class MockDiscordService:
    ...
    async def send_approval_request(self, task_id, approval_id, task_title, task_description, task_priority):
...
async def print_task_details(session, task_id):
    ...
    if task:
        print(f"  [DB TaskRecord] ID: {task.id} | Title: '{task.title}' | Status: '{task.status}' | Priority: {task.priority}")
    else:
...
async def print_approval_details(session, task_id):
    ...
    for app in approvals:
        print(f"  [DB ApprovalRecord] ID: {app.id} | Status: '{app.status}' | Decided By: {app.decided_by} | Reason: '{app.decision_reason}'")
...
async def print_execution_details(session, task_id):
    ...
    for ex in executions:
        print(f"  [DB ExecutionRecord] ID: {ex.id} | Runner: {ex.runner} | Exit Status: '{ex.exit_status}'")
        logs = ex.logs or ""
        ...
        for st in steps:
            print(f"      [DB Step] Step ID: {st.id} | Status: '{st.status}' | Exit Code: {st.exit_code}")
...
async def print_audit_logs(session):
    ...
    for rec in records[-10:]:
        print(f"    - Event: '{rec.event_type}' | Actor: '{rec.actor}' | Component: '{rec.component}'")
...
async def run_happy_path(engine):
    ...
    async with root_session_factory() as root_session:
        ...
        openrouter_client.complete = AsyncMock(return_value="**Execution Success Report**\n- Spawned echo command successfully.\n- Completed in 1 step.")
        ...
        approval_service = ApprovalService(shared_session, memory_service, [111222333], event_gateway)
        ...
        print(f"  - Summary report messages posted: {len([m for m in discord_service.posted_messages if m['channel'] == 'summaries'])}")
...
async def run_scenario_a_expiration(engine):
    ...
    async with root_session_factory() as root_session:
        ...
        task = await task_service.create_task(title="Temp Task", description="cmd:echo 'Runs'", priority=2)
        await task_service.change_status(task.id, TaskStatus.QUEUED)
        await root_session.flush()
        ...
        print(f"  - Prior to expiry check: Task Status: '{task.status}', Approval Status: '{approval.status}'")
...
async def run_scenario_b_execution_failure(engine):
    ...
    async with root_session_factory() as root_session:
        ...
        openrouter_client = MagicMock(spec=OpenRouterClient)
        openrouter_client.complete = AsyncMock(return_value="**Execution Failure Report**\n- Runner returned failure code.")
...
async def run_scenario_d_restart_recovery(engine_url):
    ...
    async with session_factory1() as root_session1:
        ...
        task = await task_service1.create_task(title="Recoverable Deploy", description="cmd:echo 'Ready'", priority=3)
    ...
    async with session_factory2() as root_session2:
        ...
        event_gateway2 = EventGateway()
        memory_service2 = MemoryService(shared_session2)
        task_service2 = TaskService(shared_session2, memory_service2, event_gateway2)
