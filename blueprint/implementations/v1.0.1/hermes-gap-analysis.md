# Hermes Gap Analysis (AP-105)

> The delta between what Hermes **claims/appears** to do and what it **actually** does, organized by
...
| **Reality** | `web_search` returns canned text (`hermes.py:76-86`); the plan is a hardcoded literal (`hermes.py:147-151`); in default config the entire decision sequence is hardcoded (`hermes.py:193-211`). |
...
| **Reality** | `from unittest.mock import AsyncMock` at module top (`hermes.py:7`); `is_mocked` branch in the live `execute_goal` (`hermes.py:186-211`). |
...
| **Reality** | `execute_goal` returns `exit_code: 0` unconditionally (`hermes.py:284-289`); even an in-loop exception sets `finished=True` and is recorded as a completed step (`hermes.py:242-247`), then the orchestrator finalizes `SUCCESS` (`orchestrator.py:227`). |
...
| **Reality** | Checkpoints are write-only; `execute_goal` always restarts fresh (`hermes.py:138-139`); no `resume_goal` for the agent path (only `research.py:361`/`briefing.py:250`). |
| **Impact** | An interrupted Hermes run restarts from zero; the checkpoint table implies a capability that does not exist. |
...
| **Reality** | `terminate()` is `pass` (`hermes.py:312-314`) and is never invoked by the orchestrator's agent branch (CLI runners *do* call it, `claude.py:124`). |
...
| **Reality** | Free-text LLM completion parsed by string-splitting code fences + `json.loads`, with a keyword-heuristic fallback that defaults to `finish` (`hermes.py:213-234`). |
...
| **Reality** | `read_file`/`write_file` use raw host FS with no path confinement (`hermes.py:88-105`); `execute_command` uses the sandbox but the default sandbox is `local` = no isolation (A-006, `config.py:133-137`). |
...
| **Reality** | `initialize()` checks for a key then `pass` if absent (`hermes.py:47-55`); `max_steps=5` hardcoded (`hermes.py:153`). |
...
| | |
|---|---|
| **Appears** | Hermes is tested (4 tests). |
| **Reality** | `test_hermes.py` runs entirely through the **mock** path; it asserts persistence/governance/artifact shape, never real LLM reasoning, real search, termination, or resume. |
| **Impact** | Green tests do not evidence the autonomous behaviors Hermes advertises. |
...
Sequencing of any remediation is deliberately out of scope (see `hermes-roadmap-boundary.md`).
