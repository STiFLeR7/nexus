...
S-4 added a new confinement seam and routed Hermes file tools through it without altering any
behavior the suite already relied on:

- **Hermes existing tests** (`test_hermes.py`) green: the mock-path `write_file` writes
...
| Workspace escape fails | `test_absolute_escape_denied`, `test_hermes_read_escape_denied`, `test_hermes_write_escape_denied` |
| Approved workspace access succeeds | `test_valid_relative_path_allowed`, `test_hermes_read_within_workspace_succeeds`, `test_hermes_write_within_workspace_succeeds` |
...
| `nexus/execution/runners/hermes.py` | source (file-tool confinement + `_workspace_cwd` helper; **no new tools, no feature expansion**) |
...
config defaults. No Track-H Hermes work (search/planning/cancellation/resume).
